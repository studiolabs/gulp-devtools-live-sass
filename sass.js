...
var SassUnpackModule = require("devtools-live-sass-unpack");
...
SassDevTools.prototype.loadMap = function(sassMap) {
  ...
  for (var i in map) {
    ...
    var sassDevToolsTmpFile = new SassDevToolsFile(
      this.devtoolsLive,
      map[i],
      this
    );
...
SassDevToolsFile.prototype.cleanSourceMap = function(
  sassContent,
  sourceContent
) {
  ...
  consumer.eachMapping(
    function(m) {
      ...
      var filepath = Module._findPath(path, [
        sassUnpack.rootDir,
        sassUnpack.sourceDir
      ]).replace(sassUnpack.sourceDir, "");
    ...
    {},
    consumer.ORIGINAL_ORDER
  );

  for (var i in sourcemap.sources) {
    var path = sourcemap.sources[i]
      .replace(sourcemap.sourceRoot, "")
      .replace(sassUnpack.rootDir, "");
...
SassDevToolsFile.prototype.pushFile = function(sassContent) {
  ...
  if (this.file.content === undefined) {
    ...
    record.resourceName = this.devtoolsLive.getClientHostname() +
      "/" +
      this.file.src;
